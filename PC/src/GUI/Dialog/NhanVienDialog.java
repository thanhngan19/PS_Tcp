...
public class NhanVienDialog extends JDialog {
    ...
    private NhanVienTrans nv;
    private Customer nhanVien;
    private ICustomerHandle customer= new CustomerHandle();
    private ISocketClient conn = new Log_In();
    public NhanVienDialog(NhanVienTrans nv, JFrame owner, boolean modal, String title, String type) {
    ...
    public NhanVienDialog(NhanVienTrans nv, JFrame owner, boolean modal, String title, String type, Customer nhanVien) {
        super(owner, title, modal);
        this.nv = nv;
                ...
                try {
                    if (ValidationInput()) {
                            ...
                            try {
                                ...
                                Customer nV = new Customer( txtName, txt_gender, sqlDate, txtSdt, txtEmail, 1);
                                customer.addPhone(new ListTransfer("add",nV));
                                nv.loadDataToTable(conn.findAll().getCustomerList());
                                dispose();
                ...
                try {
                    if (ValidationInput()) {
                        try {
                            ...
                            java.sql.Date sqlDate = new java.sql.Date(birthDay.getTime());
                            Customer nV = new Customer( txtName, txt_gender, sqlDate, txtSdt, txtEmail, 1);
                            customer.editPhone(new ListTransfer("edit",nV));
                            nv.loadDataToTable(conn.findAll().getCustomerList());
                            dispose();
                            dispose();
                        } catch (ParseException ex) {
